...
def mps_is_available() -> bool:
    """Return whether PyTorch MPS is usable in this environment."""
    return bool(
...
def resolve_torch_device(
    ...
    *,
    ...
    """Resolve a DeRegiME device request into a concrete torch.device."""
    requested_str = str(requested or "auto").lower()
    cuda_ok = torch.cuda.is_available() if cuda_available is None else cuda_available
    mps_ok = mps_is_available() if mps_available is None else mps_available
    ...
    if requested_str == "auto":
        ...
        return resolved, {
            "requested_device": requested_str,
    ...
    if requested_str.startswith("cuda"):
        if cuda_ok:
            resolved = torch.device(requested_str)
        elif allow_fallback:
            ...
            meta.update({"requested_device": requested_str, "fallback_used": True})
            return resolved, meta
        else:
            raise RuntimeError("Requested CUDA for DeRegiME, but CUDA is not available.")
    elif requested_str == "mps":
        if mps_ok:
            resolved = torch.device("mps")
        elif allow_fallback:
            resolved, meta = resolve_torch_device(
            ...
            raise RuntimeError("Requested MPS for DeRegiME, but MPS is not available.")
    elif requested_str == "cpu":
        resolved = torch.device("cpu")
    else:
        raise ValueError(
            f"Unsupported DeRegiME device '{requested}'. Use auto, cuda, mps, or cpu."
        )
