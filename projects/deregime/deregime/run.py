...
from .config import CONFIGS, ConfigDict, mps_is_available, resolve_torch_device
from .data import (
    _dataloader_kwargs,
    load_long_to_wide,
...
def _mps_diagnostics(device: str) -> Dict[str, Any]:
    """Return MPS diagnostics for this process."""
    return {
        "built": bool(torch.backends.mps.is_built()),
        "available": bool(mps_is_available()),
        "active": str(device) == "mps",
    }
...
def _write_runtime_diagnostics(
    ...
    *,
    ...
    diagnostics = {
        ...
        "requested_device": str(config.get("requested_device", config.device)),
        "resolved_device": str(config.device),
        "device_fallback_used": bool(config.get("device_fallback_used", False)),
        "device": str(config.device),
        "cuda": _cuda_memory_diagnostics(config.device),
        "mps": _mps_diagnostics(config.device),
...
def configure_runtime_device(config: ConfigDict) -> ConfigDict:
    """Resolve ``config.device`` and annotate diagnostics fields in-place."""
    requested = str(config.get("device", "auto"))
    resolved, meta = resolve_torch_device(
        requested,
        allow_fallback=bool(config.get("allow_device_fallback", False)),
    )
    config.requested_device = meta["requested_device"]
    config.device = str(resolved)
    config.resolved_device = str(resolved)
    config.device_fallback_used = bool(meta["fallback_used"])
    config.cuda_available = bool(meta["cuda_available"])
    config.mps_available = bool(meta["mps_available"])
    return config


def build_regime_model_and_likelihood(
    config: ConfigDict,
    series_dim: int,
    target_dim: int,
    tmark_dim: int,
):
    """Build the main DeRegiME regime-GP model and configured likelihood."""
    ...
    if (
        ...
        likelihood = RegimeStudentTLikelihood(
            ...
            tau_init_jitter=float(config.get("regime_tau_init_jitter", 0.0)),
            df_init_jitter=float(config.get("regime_df_init_jitter", 0.0)),
        ).to(config.device)
    ...
    elif config.use_kernel_heteroskedastic_noise:
...
def run_single_experiment(
    ...
    configure_runtime_device(config)
    ...
    print(f"\n=== Running experiment: {exp_name}{mode_banner} | "
          f"run_dir={run_dir} ===")
    print(
        f"INFO: DeRegiME device requested={config.requested_device} "
        f"resolved={config.device} fallback={config.device_fallback_used}"
    )
    ...
    if not pending_seeds:
        ...
        while seed_queue or active_procs:
            ...
            while seed_queue and available_gpus:
                ...
                if overrides.get("allow_device_fallback", False):
                    cmd.append("--allow-device-fallback")
                if test_only:
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help="Torch device for DeRegiME. auto chooses CUDA, then MPS, then CPU.",
    )
    parser.add_argument(
        "--allow-device-fallback",
        action="store_true",
        help="Allow explicit cuda/mps requests to fall back to another available device.",
    )
    parser.add_argument(
    ...
    def with_cli_runtime_overrides(overrides: Dict[str, Any]) -> Dict[str, Any]:
        out = dict(overrides or {})
        if args.device is not None:
            out["device"] = args.device
        if args.allow_device_fallback:
            out["allow_device_fallback"] = True
        return out
    ...
    if args.run_one:
        ...
        if target_exp is None:
            ...
            sys.exit(1)
        overrides = with_cli_runtime_overrides(target_exp.get("overrides", {}))
        seed_overrides = {**overrides, "seed": seed}
        if args.test_only:
    ...
    if args.experiments_json:
        ...
        for exp in experiments:
            name = exp.get("name")
            overrides = with_cli_runtime_overrides(exp.get("overrides", {}))
        ...
        overrides = with_cli_runtime_overrides(overrides)
        if args.test_only:
