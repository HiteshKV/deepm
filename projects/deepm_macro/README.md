...
deepm/                      # Core library
  ...
  deregime_bridge.py        #   Optional DeRegiME sidecar feature bridge
...
scripts/                    # Reproduction scripts
  ...
  build_deregime_sidecar.py #   Optional DeRegiME sidecar feature cache builder
...
```bash
bash scripts/run_live_background.sh start
bash scripts/run_live_background.sh status
# bash scripts/run_live_background.sh stop
```

The detached `screen` session survives terminal closure and computer sleep. The
daemon resumes any missing dates when the computer wakes.

The daemon reads the paper ledger on every start. If the computer was asleep or
the process was stopped, it downloads one current EOD snapshot and replays every
missing weekday sequentially before returning to the normal daily schedule.
Runs are idempotent, and catch-up stops at the first failed day so later P/L
cannot be calculated from a broken account sequence.

On macOS, install the daemon as a per-user `launchd` service so it starts at
login, restarts after a crash, and survives terminal closure:

```bash
bash scripts/install_live_launchd.sh
launchctl print "gui/${UID}/com.deepm.ibkr-paper"
```

Logs are written to
`live_state/logs/ibkr-paper-launchd.log` and
`live_state/logs/ibkr-paper-launchd.err.log`. To stop it while retaining a
recoverable copy of the service definition:

```bash
bash scripts/uninstall_live_launchd.sh
```

macOS may deny a LaunchAgent access to repositories stored under the protected
`Documents` folder. If the installer reports `Operation not permitted`, move
the checkout to a non-protected development folder or grant the shell/Python
binaries Full Disk Access, then rerun the installer. The detached `screen`
runner remains available in the meantime, but unlike `launchd` it does not
restart after logout or reboot.

Run an idempotent catch-up manually:

```bash
../../.venv/bin/python -m deepm.live.catchup \
  --mode ibkr-paper \
  --config configs/live/deepm_gat_ibkr.yaml
```
...
live_runs/YYYY-MM-DD/report_legacy.html
live_runs/YYYY-MM-DD/report_detailed.html
live_runs/YYYY-MM-DD/status.json
live_state/trading_ledger_ibkr_paper.sqlite
```

`report.html` is the user-friendly daily email summary: orders first, fills,
current paper positions, account impact, plain-English notes, and the model
signals last. `report_legacy.html` keeps the original compact technical report
layout. `report_detailed.html` is the newer detailed audit view with mapped
instrument names, targets, risk checks, and technical status.
...
Regenerate reports from an existing immutable run without placing trades:

```bash
.venv/bin/python -m deepm.live.report \
  --mode ibkr-paper \
  --date 2026-06-25 \
  --regenerate
```
...
### Optional DeRegiME sidecar features

The DeRegiME integration is intentionally a sidecar feature experiment. It does
not replace DeePM's portfolio-utility objective and does not add a separate
forecast-then-optimise allocator. The bridge writes causal `dg_` columns into a
separate feature cache, then the `deepm-gat-dg-*` configs test whether DeePM can
benefit from that probabilistic regime state.

Build the sidecar cache:
...
The default builder uses a fast causal proxy with the same DeRegiME sidecar
interface: expected return, uncertainty, tail risk, and four-regime
probabilities at horizons 1, 5, 21, and 63. It uses the DeePM `target` column
with a one-row lag, so same-day model inputs cannot see the next-day return.
If full DeRegiME prediction artifacts are available, compile them instead:

```bash
.venv/bin/python scripts/build_deregime_sidecar.py \
  --method artifacts \
  --artifacts-predictions-dir path/to/deregime/results/predictions
```

For a full rolling DeRegiME training protocol aligned to DeePM, generate the
rolling experiment JSON and train that file:

```bash
.venv/bin/python scripts/build_deregime_rolling_experiments.py

.venv/bin/python -m deregime.run \
  --experiments_json data/deregime/deepm_deregime_rolling_experiments_20260625.json \
  --out_dir deregime_runs
```

That produces one DeRegiME experiment per DeePM test window:

```text
2010 window: train/valid dates < 2010-01-01, test 2010-2014
2015 window: train/valid dates < 2015-01-01, test 2015-2019
2020 window: train/valid dates < 2020-01-01, test 2020-2026 available data
```

The original `data/deregime/deepm_deregime_experiments_20260625.json` is a
single ratio-split DeRegiME run and is useful only as a quick full-model smoke
attempt, not as the leakage-safe rolling sidecar protocol.

After those rolling DeRegiME experiments finish, compile their prediction and
activation artifacts into the DeePM sidecar cache:

```bash
.venv/bin/python scripts/build_deregime_rolling_sidecar.py
```

This writes `data/feats-data_20260625_deregime.parquet` from the real rolling
DeRegiME artifacts. Until this command is run successfully, the current
`feats-data_20260625_deregime.parquet` may still be the fast proxy sidecar.

Train and evaluate the ablations:

```bash
.venv/bin/python -m deepm.training -r deepm-gat-dg-mean -a DeePM
.venv/bin/python -m deepm.training -r deepm-gat-dg-risk -a DeePM
.venv/bin/python -m deepm.training -r deepm-gat-dg-full -a DeePM

.venv/bin/python -m deepm.backtest --name bt-deepm-gat-dg-mean --diagnostics
.venv/bin/python -m deepm.backtest --name bt-deepm-gat-dg-risk --diagnostics
.venv/bin/python -m deepm.backtest --name bt-deepm-gat-dg-full --diagnostics

.venv/bin/python scripts/aggregate_metrics.py \
  --title "DeRegiME Sidecar Ablation: Yahoo Proxy Data (2010-2026)" \
  --csv backtest_results/current_20260625_deregime_ablation_metrics.csv \
  bt-deepm-gat bt-deepm-gat-dg-mean bt-deepm-gat-dg-risk bt-deepm-gat-dg-full

.venv/bin/python scripts/generate_deregime_ablation_report.py
```

Only promote a sidecar variant if the trading metrics improve versus
`bt-deepm-gat` and the audit file confirms the causal feature policy.

### Distinct experiment regimes

The repo now keeps the three active research lanes separate while preserving a
shared evaluator:

| Regime | Train config(s) | Architecture | Backtest config(s) | Use when |
|--------|-----------------|--------------|--------------------|----------|
| Original DeePM | `deepm-gat` | `DeePM` | `bt-deepm-gat-k10-current` | You want the base model/control. |
| DeRegiME DeePM | `deepm-gat-dg-mean`, `deepm-gat-dg-risk`, `deepm-gat-dg-full` | `DeePM` | `bt-deepm-gat-dg-*` | You want causal `dg_` sidecar feature ablations. |
| Momentum DeePM | `deepm-mt-vsn` | `MT_DEEPM` | `bt-deepm-mt-vsn-k10-current` | You want the Momentum Transformer-style temporal variant. |

Print commands for one lane without starting it:

```bash
.venv/bin/python scripts/experiment_regime.py list
.venv/bin/python scripts/experiment_regime.py commands original
.venv/bin/python scripts/experiment_regime.py commands deregime
.venv/bin/python scripts/experiment_regime.py commands momentum
.venv/bin/python scripts/experiment_regime.py commands momentum --stage train -fsy 2020
```

Print one combined comparison command:

```bash
.venv/bin/python scripts/experiment_regime.py combined
```

The Momentum lane uses the same DeePM portfolio loss, transaction costs,
rolling windows, validation split, and K=10 seed-ensemble backtester as the
control. Its artifacts are isolated under
`models_torch/deepm-mt-vsn/MT_DEEPM/`.

The Momentum lane has an explicit training device setting. On the local Apple
Silicon machine, the benchmark selected MPS for `MT_DEEPM`:

```text
CPU: 4.889 sec/train batch
MPS: 0.741 sec/train batch
CUDA: unavailable
```

Re-run the benchmark after changing hardware, PyTorch, or batch size:

```bash
.venv/bin/python scripts/benchmark_deepm_device.py \
  -r deepm-mt-vsn \
  -a MT_DEEPM \
  --devices cpu mps \
  --output reports/deepm_mt_vsn_20260625/device_benchmark.json
```

After Momentum checkpoints exist, export feature-selection and temporal
attention diagnostics:

```bash
.venv/bin/python scripts/export_momentum_interpretability.py \
  --backtest bt-deepm-mt-vsn-k10-current
```
...
`report.html` is the readable daily summary intended for email. It explains the
paper account movement, orders, fills, and current exposure using instrument
names instead of only model tickers. `report_legacy.html` is the original
technical report layout. `report_detailed.html` is the newer audit report with
mapped instrument names, risk checks, raw targets, and the full signal table.
...
Configs may optionally use `inherits: <config-name>` to inherit another YAML and
override only changed fields. This is used by the DeRegiME and Momentum lanes so
the ticker universe, costs, rolling windows, and model defaults stay aligned with
their controls.
...
| `deepm-gat-dg-mean` | DeePM + DeRegiME sidecar | Mean/probability sidecar ablation |
| `deepm-gat-dg-risk` | DeePM + DeRegiME sidecar | Uncertainty/tail-risk sidecar ablation |
| `deepm-gat-dg-full` | DeePM + DeRegiME sidecar | Full sidecar ablation |
| `deepm-mt-vsn` | Portfolio Momentum Transformer | Base-DeePM Momentum Transformer integration |
...
| `bt-deepm-gat-dg-mean` | DeRegiME mean/probability sidecar | Sidecar feature ablation |
| `bt-deepm-gat-dg-risk` | DeRegiME uncertainty/tail-risk sidecar | Sidecar feature ablation |
| `bt-deepm-gat-dg-full` | Full DeRegiME sidecar | Sidecar feature ablation |
| `bt-deepm-mt-vsn-k10-current` | Portfolio Momentum Transformer | Base-DeePM Momentum Transformer integration |
