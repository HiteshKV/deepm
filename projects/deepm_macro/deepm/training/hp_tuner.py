...
class Tuner:
    ...
    def _resume_or_create_sweep(self):
        ...
        remaining = self.settings["random_search_max_iterations"] - num_completed
        sweep_id = self._select_available_sweep(runs, required_jobs=remaining)
        if sweep_id is None:
            sweep_id = self._create_sweep()
    ...
    def _select_available_sweep(self, runs: list[Any], required_jobs: int = 1) -> str | None:
        ...
        for sweep_id, items in sorted(
            ...
            if search_space_size is None:
                return sweep_id
            available_jobs = search_space_size - len(items)
            if available_jobs >= required_jobs:
                return sweep_id
            self.logger.info(
                "Sweep %s has %d/%d jobs already and only %d available grid "
                "slots; %d usable local runs are still needed.",
                sweep_id,
                len(items),
                search_space_size,
                max(0, available_jobs),
                required_jobs,
            )
