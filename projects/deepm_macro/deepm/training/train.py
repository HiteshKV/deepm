...
from deepm.models.momentum_transformer import (
    MomentumTransformer,
    PortfolioMomentumTransformer,
)
...
def resolve_training_device(requested: str | None = None) -> torch.device:
    """Resolve an explicit or automatic training device request."""
    requested = (
        requested
        or os.environ.get("DEEPM_TRAIN_DEVICE")
        or os.environ.get("DEEPM_DEVICE")
        or "auto"
    )
    ...
    if requested in {"cuda", "cuda:0"}:
        if not torch.cuda.is_available():
            raise RuntimeError("Requested CUDA for DeePM training, but CUDA is unavailable.")
        return torch.device("cuda:0")
    if requested == "mps":
        if not torch.backends.mps.is_available():
            raise RuntimeError("Requested MPS for DeePM training, but MPS is unavailable.")
        return torch.device("mps")
    raise ValueError("Unknown DeePM training device request: %s" % requested)


def get_training_device() -> torch.device:
    """Return the configured DeePM training device."""
    return resolve_training_device()


def set_training_device(requested: str | None = None) -> torch.device:
    """Set the module-global DeePM training device."""
    global device
    device = resolve_training_device(requested)
    return device


def get_current_training_device() -> torch.device:
    """Return the current module-global DeePM training device."""
    return device


def available_training_devices() -> list[str]:
    """Return locally available DeePM training backends."""
    devices = ["cpu"]
    if torch.cuda.is_available():
        devices.append("cuda")
    if torch.backends.mps.is_available():
...
class TrainDeepMomentumNetwork:
    ...
    ARCHITECTURE_CLASSES = {
        ...
        "MT_DEEPM": PortfolioMomentumTransformer,
    }
