...
from deepm.training.train import set_training_device
...
def run(
    ...
    if settings.get("device"):
        set_training_device(settings["device"])
...
def _parse_args():
    ...
    parser.add_argument(
        "--device",
        choices=["auto", "cpu", "mps", "cuda", "cuda:0"],
        default=None,
        help="Training device override. Defaults to config/env auto selection.",
    )
...
def main():
    ...
    set_training_device(settings.get("device"))
