...
def build_orders(
    ...
    for target in targets:
        ...
        reason = "rebalance"
        if account_equity is not None and max_order_notional > 0 and target.contract_value > 0:
            max_delta_contracts = int((account_equity * max_order_notional) // target.contract_value)
            if max_delta_contracts <= 0:
                continue
            if abs(delta) > max_delta_contracts:
                effective_delta = int(math.copysign(max_delta_contracts, delta))
                reason = "rebalance_order_delta_capped_to_max_notional"

        price_offset = max(ticks * instrument.tick_size, abs(target.market_price) * bps / 10000.0)
        side = "BUY" if effective_delta > 0 else "SELL"
        ...
        orders.append(
            OrderIntent(
                ...
                side=side,
                quantity=abs(effective_delta),
                current_quantity=target.current_quantity,
                target_quantity=target.current_quantity + effective_delta,
