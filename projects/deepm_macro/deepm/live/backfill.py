...
def run_backfill(
    ...
    """Replay daily runs in order and write a compact summary CSV.

    The replay stops at the first unsuccessful date. Continuing after a failed
    day would make subsequent positions and P/L diverge from a continuous
    paper account.
    """
    ...
    for run_date in business_dates(start_date, end_date):
        ...
        accepted_statuses = {"completed", "already_ran"}
        if no_place_orders:
            accepted_statuses.add("dry_run")
        if status.get("status") not in accepted_statuses:
            failed_date = run_date
            failed_errors = " | ".join(status.get("errors", []))
            break
    ...
    if failed_date is not None:
        raise RuntimeError(
            "Sequential backfill stopped at "
            f"{failed_date.isoformat()}: {failed_errors or 'daily run did not complete'}; "
            f"partial audit: {output}"
        )
    return output
