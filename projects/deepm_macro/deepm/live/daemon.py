...
from deepm.live.catchup import catch_up
...
def run_once(args: argparse.Namespace) -> Path | None:
    ...
    uses_local_paper_ledger = args.mode == "sim" or (
        args.mode == "ibkr-paper"
        and bool(config.get("ibkr", {}).get("paper_simulation", True))
    )
    if uses_local_paper_ledger and not args.date and not args.run_now:
        return catch_up(
            mode=args.mode,
...
def main() -> None:
    ...
    print(f"DeePM live daemon started mode={args.mode} poll_seconds={poll_seconds}", flush=True)
    while True:
        try:
            report = run_once(args)
            if report:
